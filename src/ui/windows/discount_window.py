...
def _elide_to_lines(text: str, font, width: int, *, max_lines: int) -> str:
    ...
    # 文本本身不足 max_lines：直接返回原文（允许自然换行）
    if len(line_slices) < max_lines and sum(l for _, l in line_slices) >= len(text):
        return text

    # 前 max_lines-1 行：保留；最后一行：把剩余全部塞进来并做 elide
    kept_lines: list[str] = []
...
class DiscountWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 初始化为 UI 空态/占位；后续由外部调用 update_* 方法填充数据。
    ...
    def _build_news_tab(self) -> QWidget:
        ...
        # 避免 QLabel 在 VBoxLayout 中被“拉伸”导致标题区出现大片空白
        ...
        self.news_detail.setHtml(
            "<h3>暂无新闻</h3>"
            "<p>此页仅实现 UI：后续接入 RSS 后，将在左侧列表展示新闻条目，并在每条条目中标注 <b>源：RSS源名称</b>。</p>"
        )
        splitter.addWidget(self.news_detail)

        splitter.setStretchFactor(0, 3)
        splitter.setStretchFactor(1, 5)
        # 让 splitter 占满剩余空间（避免内容只占一半高度）
    ...
    def update_news_data(self, news_items: list[dict]):
        """仅 UI：更新新闻列表。每条新闻必须包含/展示 source（RSS 源名称）。"""
        ...
        if not self._news_items:
            item = QListWidgetItem("暂无新闻（UI占位）")
            item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _on_news_selection_changed(self, row: int):
        ...
        if summary:
            parts.append(f"<hr/><p>{summary}</p>")
        else:
            parts.append("<hr/><p>（此处为正文/摘要占位）</p>")
